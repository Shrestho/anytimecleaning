...
class PaymentAPIController extends Controller
{
    ...
    public function payment(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $input['payment']['amount'] = $booking->getTotal();
            $input['payment']['description'] = __('lang.payment_booking_id') . $input['id'];
            $input['payment']['payment_status_id'] = 2; //paid
            $input['payment']['payment_method_id'] = $input['payment_method_id'];
            $input['payment']['user_id'] = $booking->user_id;
            $payment = $this->paymentRepository->create($input['payment']);
            $booking = $this->bookingRepository->update(['payment_id' => $payment->id], $input['id']);
            // Notification::send($booking->e_provider->users, new StatusChangedPayment($booking));
    ...
    {
        \Stripe\Stripe::setApiKey('sk_test_51LPIPWB1kF8HAmvfcoEXCOj06Jw35GdKW2uUpSsgZaGT2cZwNnjvevB4MDlzv2FqrYkdzZMena6fotQBqc2wdkmo00pvj6s2Bl');
        ...
        $paymentIntent = PaymentIntent::create([
            'amount' => $amount,
            'currency' => $currency,
            // Add any additional payment intent options here, such as description or metadata
        ]);
        
        // Return the payment intent client secret to the client-side code
        // return response()->json(['client_secret' => $paymentIntent->client_secret]);
        return $this->sendResponse($paymentIntent->client_secret, __('Payment intent Create successfully'));
    }
}
