...
class EServiceController extends Controller
{
    ...
    {
        // $eService = EService::find(1); 
        // $providers = $eService->providers()->pluck('name')->implode(', ');
        // dd($providers);
    ...
    public function create()
    {
        ...
        $eProvider = $this->eProviderRepository->whereHas("roles", function($q){ $q->where("name", "provider"); })->getByCriteria(new EProvidersOfUserCriteria(auth()->id()))->pluck('name', 'id');
        $categoriesSelected = [];
    ...
    // public function providerServicesCreate()
    // {
    //     dd($service);
    //     return view('e_services.provider_create');
    // }
