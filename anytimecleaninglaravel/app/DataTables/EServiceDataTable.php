...
class EServiceDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('providers', function ($eService) {
                return getLinksColumnByRouteName($eService->providers, 'eProviders.edit', 'id', 'name');
            })
    ...
    public function query(EService $model)
    {
        if (auth()->user()->hasRole('provider')) {
            return $model->newQuery()->join('users', 'users.id', '=', 'e_services.user_id')
                ->groupBy('e_services.id')
                ->where('users.id', auth()->id())
                ->select('e_services.*')
                ->orderBy('updated_at', 'desc');
        }
        return $model->newQuery()->select("$model->table.*")->orderBy('updated_at', 'desc');
    }
