...
class ExperienceDataTable extends DataTable
{
    ...
    public function query(Experience $model)
    {
        if (auth()->user()->hasRole('provider')) {
            return $model->newQuery()->join('users', 'users.id', '=', 'experiences.user_id')
                ->groupBy('experiences.id')
                ->select('experiences.*')
                ->where('users.id', auth()->id())->orderBy('updated_at', 'desc');
        } else {
            return $model->newQuery()->with("eProvider")->select("$model->table.*")->orderBy('updated_at', 'desc');
        }
    }
