...
class RequestedEProviderDataTable extends DataTable
{
    ...
    //         return $model->newQuery()->with("eProviderType")->where('users.accepted', '0')->select("$model->table.*");
    //     } else {
    //         return $model->newQuery()
    //             ->with("eProviderType")
    //             ->join("users", "id", "=", "e_providers.id")
    ...
    public function query(User $model)
    {
        ...
        return $model->newQuery()->with('roles')->where('users.accepted', '0')->whereHas('roles', function($query) {
