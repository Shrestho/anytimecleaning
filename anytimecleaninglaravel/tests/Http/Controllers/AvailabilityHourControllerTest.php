...
class AvailabilityHourControllerTest extends TestCase
{
    ...
    public function testStore()
    {
        ...
        $this->assertDatabaseHas(AvailabilityHour::getModel()->table, [
            ...
            'user_id' => $availabilityHour->user_id
        ]);
    ...
    public function testRequiredFieldsWhenStore()
    {
        ...
        $response->assertSessionHasErrors("user_id", __('validation.required', ['attribute' => 'user_id']));
    }
    ...
    {
        $user = TestHelper::getAdmin();
        $availabilityHour = factory(AvailabilityHour::class)->states(['day_more_16_char', 'not_exist_user_id', 'end_at_lest_start_at'])->make();
        $response = $this->actingAs($user)
            ->post(route('availabilityHours.store'), $availabilityHour->toArray());
        ...
        $response->assertSessionHasErrors("user_id", __('validation.exists', ['attribute' => 'user_id']));
    }
